...
async function updatePeriodDefinitions() {
    let pool;
    try {
        ...
        console.log(`Found ${sourcePeriods.length} period definitions to copy.`);

        // 2. Start a transaction to ensure all or nothing is updated
        ...
        try {
            ...
            for (const period of sourcePeriods) {
                ...
                // Check if the period already exists for the target school
                const checkResult = await new sql.Request(transaction)
                    .input('TargetSchoolCode', sql.NVarChar, TARGET_SCHOOL_CODE)
                    .input('PeriodNo', sql.Int, period.PeriodNo)
                    .query('SELECT COUNT(*) as count FROM perioddef WHERE SchoolCode = @TargetSchoolCode AND PeriodNo = @PeriodNo');

                const exists = checkResult.recordset[0].count > 0;

                if (exists) {
                    // Update existing period
                    console.log(`Updating Period No: ${period.PeriodNo} for school ${TARGET_SCHOOL_CODE} (${formattedStartTime} - ${formattedEndTime})`);
                    await new sql.Request(transaction)
                        ...
                        .query(`
                            ...
                            WHERE SchoolCode = @TargetSchoolCode AND PeriodNo = @PeriodNo
                        `);
                    updatedCount++;
                } else {
                    // Insert new period
            ...
            console.log('\n--- SUCCESS ---');
            console.log(`Successfully updated ${updatedCount} and inserted ${insertedCount} period definitions for school ${TARGET_SCHOOL_CODE}.`);
            console.log('---------------');
